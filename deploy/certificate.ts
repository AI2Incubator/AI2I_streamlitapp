...
export const getCertificateValidation = (
   ...
   */
  const hostedZoneId = aws.route53.getZone({ name: domain }).then(zone => zone.zoneId)
  const route53ValidationRecords = certificate.domainValidationOptions.apply(options =>
    options.map(
      option =>
